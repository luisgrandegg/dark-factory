...
# Construct the fake key at runtime so this doc itself doesn't trip
# gitleaks. AWS's published example is AKIA + 16 chars; we split it.
printf '\nAWS_KEY="%s%s"\n' 'AKIA' 'IOSFODNN7EXAMPLE' >> scripts/leak.sh
